...
func TestGraph_ll(t *testing.T) {
	...
	for i := 0; i < len(expectedAdjMatrix); i++ {
		for j := 0; j < len(expectedAdjMatrix[i]); j++ {
			if g.Get_edge_w(i, j) != expectedAdjMatrix[i][j] {
				t.Errorf("Expected adjacency matrix[%d][%d]: %d, got: %d", i, j, expectedAdjMatrix[i][j], g.Get_edge_w(i, j))
			}
		}
	}
	...
	for i := 0; i < len(expectedAdjMatrixDirected); i++ {
		for j := 0; j < len(expectedAdjMatrixDirected[i]); j++ {
			if g.Get_edge_w(i, j) != expectedAdjMatrixDirected[i][j] {
				t.Errorf("Expected modified adjacency matrix[%d][%d]: %d, got: %d", i, j, expectedAdjMatrixDirected[i][j], g.Get_edge_w(i, j))
			}
		}
	}
